...
class ImageState {
  ...
  ImageState({this.imageQuality = false, required this.imageQualityLevel});
  ImageState copyWith({
    bool? imageQuality,
...
class ImageQualityStateNotifier extends StateNotifier<ImageState> {
  ...
  void setImageQuality(bool newQuality) {
    ...
    if (!state.imageQuality) {
      state = state.copyWith(imageQualityLevel: 80);
    } else {
      state = state.copyWith(imageQualityLevel: 100);
    }
  }
}

final imageQualityProvider =
    StateNotifierProvider<ImageQualityStateNotifier, ImageState>((ref) {
